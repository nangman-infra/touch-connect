...
- [active/README.md](docs/active/README.md)
...
- [active/governance/document-lifecycle.md](docs/active/governance/document-lifecycle.md)
  - 상태 전이, 인덱스 규칙, 파일명/이동 규칙
- [templates/document-template.md](docs/templates/document-template.md)
  - 상태 관리 문서가 아닌 문서 작성용 템플릿

## 현재 active 문서

현재 active 문서 목록은 [active/README.md](docs/active/README.md)에서 관리한다.
...
- 작성일: 2026-05-04
- 관점: `A2A/AGNTCY 위에서 메시지 품질과 handoff governance를 보장하는 레이어`
- 전제:
  - 자체 큐, transport, replay 구현에 제품 시간을 쓰지 않는다.
  - production transport와 durability는 NATS JetStream, Temporal, A2A, AGNTCY-compatible adapter 뒤에 둔다.
  - memory와 SQLite 기반 store는 dev/test 경로로 강등한다.
  - 공개 wire surface는 A2A-compatible envelope 또는 그 superset으로 제한한다.
