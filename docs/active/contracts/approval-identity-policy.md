...
> Canonical Path: `/Volumes/WD/Developments/touch-connect/docs/active/contracts/approval-identity-policy.md`
> Source Of Truth: yes
...
이들은 adapter choice이고, domain contract는 `actor identity`, `role`, `capability`, `approval`만 소유한다.

## 현재 구현 기본값

- actor type은 `human_user`, `agent_instance`, `service_principal` 세 가지만 먼저 구현한다.
- approval은 boolean flag가 아니라 별도 aggregate로 구현한다.
- 모든 protected action은 approval decision record 없이 실행되면 안 된다.
