...
기준 날짜는 `2026-05-04`이다.

## 2026-05-04 Pivot Update

이번 update의 결론은 명확하다.

`touch-connect`는 queue, transport, replay를 직접 만들지 않고 A2A/AGNTCY 위의 message quality와 handoff governance를 제품 표면으로 잡는다.

외부 근거:

- Linux Foundation은 2026-04-09 A2A가 150개 이상 조직의 지원, 주요 클라우드 통합, 여러 산업의 production deployment를 확보했고 v1.0 stable specification으로 진입했다고 발표했다.
- A2A latest specification은 Task, Message, Artifact를 중심으로 agent 상호운용 surface를 제공한다.
- NATS JetStream 공식 문서는 storage/replay, work-queue retention, `Nats-Msg-Id` dedupe, consumer ack를 제공한다고 설명한다.
- Temporal은 2026-02-17 $300M Series D 발표에서 long-running stateful AI systems를 위한 durable execution layer를 전면에 세웠다.
- AGNTCY의 `agntcy/acp-spec` GitHub repository는 2026-04-11 archived 상태다. 따라서 `AGNTCY ACP 호환`은 방향성으로 유지하되, production adapter 계약은 현재 유지되는 AGNTCY source of truth를 재확인한 뒤 확정해야 한다.

이 update는 `touch-connect`가 넓은 인프라 카테고리로 확장하는 것이 아니라, 이미 표준화되는 transport/wire 생태계 위에서 비어 있는 message-quality and governance surface를 맡는다는 결정을 고정한다.
...
내 해석으로는 `touch-connect`의 production transport adapter는 MQTT보다 JetStream 류에 더 가깝고, MQTT는 edge bridge나 간헐 연결 환경에서 더 잘 맞는다.

중요한 변경:

`touch-connect`가 JetStream과 경쟁하는 queue를 만들면 안 된다. JetStream은 transport/durability/replay 후보이고, `touch-connect`는 그 위의 PhraseologyPolicy, ApprovalChain, ArtifactLineage를 맡는다.
...
2026-05-04 기준으로는 A2A 1.0이 production-ready open standard로 외부 인지를 얻고 있으므로, 독자 wire protocol을 정의할 이유가 더 줄었다.
...
- PhraseologyPolicy, CapabilityClaim, CorrelationGraph, ApprovalChain, ArtifactLineage를 제품 표면으로 올린다.
...
- JetStream, Temporal, A2A, AGNTCY-compatible adapter를 통해 production 인프라를 붙인다.

### 7.2 피해야 할 방향

- agent chat wrapper
- protocol-first product
- 자체 queue/broker/replay product
- MCP-only 내부 통신
...
`Touch Connect is the message-quality and handoff-governance layer that helps heterogeneous AI agents communicate without losing intent, constraints, approval scope, or artifact lineage.`
