...
`touch-connect`는 A2A/AGNTCY 같은 agent interoperability layer 위에서 메시지 품질과 handoff governance를 보장하는 레이어다.

30초 hook:

`A2A와 AGNTCY가 메시지를 전달하면, touch-connect는 그 메시지가 충분한지, 정확히 이해되었는지, 누가 승인했는지를 보장한다.`
...
따라서 `touch-connect`가 푸는 핵심 문제는 전송, durability, replay가 아니다.
...
- A2A/AGNTCY-compatible message 위에 message quality policy를 적용한다.
- 역할 간 handoff가 goal, constraints, next action을 readback할 수 있게 만든다.
- 메시지와 산출물, 제약, 승인, 추적 정보를 하나의 governance chain으로 묶는다.
- CapabilityClaim, CorrelationGraph, ApprovalChain, ArtifactLineage를 제품 표면으로 올린다.
- CLI 기반 작업 흐름과 skill 기반 워크플로를 우선한다.
- MCP는 외부 도구와 데이터로 나가는 edge layer로 둔다.
...
- 또 하나의 queue, broker, transport, replay engine
...
- 대규모 범용 모델 학습 플랫폼
- transport, durability, replay 자체 구현
- 독자 표준 제정
...
4. 멀티벤더 환경에서는 새로운 transport보다 A2A/AGNTCY-compatible message 위의 quality/governance layer가 더 방어 가능한 위치다.
...
- transport adapter와 message governance contract를 분리한다.
...
- 업계가 `AGENTS.md`, `Skills`, `MCP`, `A2A`, AGNTCY 류로 각 계층을 빠르게 표준화하고 있다.
