...
> Scope: checkpoint, raw history, artifact ledger, takeover, claim/lease, redelivery의 draft contract
> Canonical Path: `/Volumes/WD/Developments/touch-connect/docs/planned/contracts/checkpoint-and-takeover-model.md`
> Source Of Truth: no
...
이 문서는 `touch-connect`의 takeover, redelivery, 처리 보장 계약을 닫기 전에 필요한 초안이다.

현재 active 문서만으로는 아래가 아직 완전히 닫히지 않았다.
...
이 문서는 위 질문에 대한 현재 초안 방향을 모은다.  
아직 current contract는 아니다.
...
이 초안이 active로 승격되려면 아래 문서와 충돌 없이 접합되어야 한다.
