...
`touch-connect`는 여러 이기종 AI와 에이전트가 A2A/AGNTCY-compatible 메시지를 주고받을 때, 그 메시지의 충분성, readback, 승인 범위, artifact lineage를 보장하는 message-quality and handoff-governance layer다.

## 제품 철학

`touch-connect`의 본질은 message queue가 아니다.

이 제품은 아래를 동시에 만족해야 한다.

- PhraseologyPolicy로 메시지의 필수 의미를 검증한다
- CapabilityClaim으로 endpoint capability의 버전, 범위, fallback을 표현한다
- CorrelationGraph로 task, message, artifact, approval 관계를 잇는다
- ApprovalChain으로 승인 범위, 만료, 재승인 조건을 추적한다
- ArtifactLineage로 산출물의 parent, derived_from, supersedes 관계를 보존한다

즉, `touch-connect`는 `message quality and handoff governance layer`에 가깝다.
...
- transport, durability, replay: NATS JetStream, Temporal, A2A, AGNTCY-compatible adapter 책임
- wire protocol과 agent discovery: A2A 1.0 호환 또는 그 superset 책임
- message quality: `touch-connect` 책임
- handoff governance: `touch-connect` 책임
- tool integration: MCP 같은 외부 edge layer 책임

이 경계는 자체 queue나 독자 wire protocol을 제품 중심으로 만들지 않는다는 뜻이다.
...
- capability claim 기준의 message handoff surface
- phraseology와 readback 기반 message quality policy
- task, message, artifact, approval을 잇는 correlation graph
- approval chain과 identity binding
- artifact lineage와 provenance reference system
- adapter-backed delivery와 processing continuity projection
...
- 자체 queue, broker, transport, replay engine
- 범용 채팅앱

한 줄로 말하면, `touch-connect`는 `전송 시스템`이 아니라 `메시지 품질과 governance 시스템`이다.
...
- CapabilityClaim 기준으로 적절한 endpoint 후보를 검증
- 필수 계약 충족 여부 검증
- 처리 상태 기록
- approval, artifact lineage, side-effect evidence 연결
...
v1의 runtime ledger 1급 객체는 아래 다섯 개다.
...
제품이 외부에 강하게 드러내는 1급 surface는 아래 다섯 개다.

### PhraseologyPolicy

기존 `readback_required` boolean의 상위 모델이다.

- capability별 필수 field schema
- readback 대상
- missing-constraint 검증 규칙
- ambiguity 처리 규칙
- fallback action

현재 구현 호환을 위해 `readback_required=true`는 `PhraseologyPolicy.readback.mode=required`의 축약 표현으로 받는다.

### CapabilityClaim

기존 `target_capability` string의 상위 모델이다.

- 하나 이상의 capability name
- version constraint
- scope
- fallback chain
- endpoint claim evidence

현재 구현 호환을 위해 단일 `target_capability`는 `CapabilityClaim.capabilities[0].name`의 축약 표현으로 받는다.

### CorrelationGraph

기존 단일 `correlation_ref`의 상위 모델이다.

- task와 message edge
- message와 artifact edge
- approval과 side effect edge
- supersedes와 derived_from edge

현재 구현 호환을 위해 단일 `correlation_ref`는 `CorrelationGraph`의 `trace` edge 하나로 투영한다.

### ApprovalChain

기존 approval record의 상위 모델이다.
...
기존 artifact version list의 상위 모델이다.
...
- message 하나는 하나의 primary `CapabilityClaim`을 가진다
...
`discovery + routing + correlation + governance projection`을 함께 가진다.

### correlation

- `correlation_graph_refs`는 선택값이다
- `correlation_ref`는 현재 구현 호환 projection이다
- 보내는 AI가 넣는다
...
- `target_capability`는 단일 capability만 표현하는 `capability_claim` 축약형이다.
- `readback_required`는 `phraseology_policy.readback.mode` 축약형이다.
- `correlation_ref`는 `correlation_graph_refs` 안의 단일 trace edge 축약형이다.
...
가 약해지고, NATS JetStream이나 Temporal과 정면으로 겹친다.
...
`message quality + handoff governance`에 집중하는 제품이어야 한다.

## 최종 포지셔닝

### 내부 정의

`Message Quality and Handoff Governance Layer for Heterogeneous AI Agent Systems`

### 외부 설명

`Touch Connect helps heterogeneous AI agents hand off work without losing intent, constraints, approval scope, or artifact lineage.`
