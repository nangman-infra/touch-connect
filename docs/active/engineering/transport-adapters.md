...
- domain object 중심이라 SQLite transaction object가 직접 새지 않는다.
- endpoint, message, attempt, checkpoint, artifact, approval, side effect ledger가 한곳에 모여 있어 e2e 테스트가 쉽다.
- `ClaimNextMessage`가 worker polling contract를 application level에서 드러낸다.

정제해야 할 지점:

- `NextRef(kind string)`은 store 책임이 아니라 `RefAllocator` port 책임이다.
- `Snapshot()`은 write store 책임이 아니라 projection/query port 책임이다.
- `ReconcileExpiredClaims(now)`는 adapter timer 또는 processing ledger service로 이동해야 한다.
- `ClaimMessage`와 `ClaimNextMessage`는 transport ack와 domain claim을 분리하는 `DeliveryAdapter` plus `ProcessingLedger` 경계로 쪼개야 한다.
- 모든 `Save*` 메서드가 같은 interface에 있어 adapter별 최소 구현 단위를 과도하게 넓힌다.
...
M1 W1의 완료 기준은 위 분리를 코드에 한 번에 전부 적용하는 것이 아니라, 현재 9개 통합테스트가 adapter interface 위에서 같은 의미로 통과할 수 있도록 SQLite/memory 전용 가정이 어디에 있는지 제거하는 것이다.
